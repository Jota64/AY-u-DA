...
public:
   ...
   */
  std::vector<std::vector<Color>> remove_n_lowest_seams(size_t num_seams_to_remove) const noexcept;
...
private:
   ...
   */
  std::vector<std::vector<Color>> remove_seam(const std::vector<std::vector<Color>>& pixels, const std::vector<size_t>& seam_cols) const noexcept;
